...
describe("Octane", () => {
  ...
  it("should initialize Octane with the correct network", async () => {
    octane = new Octane({
      apiKey: "test-api-key",
      network: "beta-5",
    });
  ...
  it("should throw an error if an unsupported network is provided", async () => {
    ...
    }).toThrowError(`Network unsupported-network is not supported. It must be one of: ${supportedNetworkIds}.`);
  });
